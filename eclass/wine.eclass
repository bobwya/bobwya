...
wine_add_stock_gentoo_patches() {
	...
	case "${WINE_PV}" in
		1.8*|1.9*|2.*|3.*|4.*|5.0.[1-9]|5.0.[1-9]-rc[1-9]|5.[0-9]|5.1[0-9]|5.20|9999)
			PATCHES+=( "${_patch_directory}/wine-5.21-fix_compatibility_with_autoconf_2.70.patch" )
			;;
		*)
			;;
	esac
	...
	case "${WINE_PV}" in
		9999)
			...
			;;
		5.9|5.1[0-9]|5.2[0-2]|6.0|6.0-rc[1-9]|6.0.[1-4]|6.0.[1-4]-rc[1-9]|6.[1-9]|6.1[0-9]|6.2[0-1])
			PATCHES_REVERT+=( "${_patch_directory}/revert/wine-5.9-makedep_install_also_generated_typelib_for_installed_idl.patch" )
			;;
		*)
			;;
