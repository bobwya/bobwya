...
if [[ $(ver_cut 3) -lt 90 ]]; then
	KEYWORDS="~alpha ~amd64 ~arm ~arm64 ~hppa ~ia64 ~m68k ~mips ~ppc ~ppc64 ~s390 ~sparc ~x86 ~amd64-linux ~arm-linux ~x86-linux ~ppc-macos ~x64-macos ~x86-macos"
fi
...
src_prepare() {
	...
	default
	eautoreconf
	rm "test/out.expected" || die "rm failed" # 662048
}
