...
# Patch version
PATCH="${MOZ_PN}-10.0-patches-0.10"
...
IUSE="bindist +ipc kde +minimal pgo selinux system-sqlite +webm"
...
src_install() {
	...
	newmenu "${FILESDIR}/icon/${MOZ_PN}.desktop" "${MOZ_PN}.desktop" || die
	sed -i -e "s:@NAME@:${name}:" -e "s:@ICON@:${icon}:" \
		"${ED}/usr/share/applications/${MOZ_PN}.desktop" || die

	# Add StartupNotify=true bug 237317
	if use startup-notification ; then
		echo "StartupNotify=true" >> "${ED}/usr/share/applications/${MOZ_PN}.desktop"
	fi
