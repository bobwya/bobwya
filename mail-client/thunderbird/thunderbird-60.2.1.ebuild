...
src_prepare() {
	...
	if use kde; then
		# Gecko/toolkit OpenSUSE KDE integration patchset (w/ hack for thunderbird-60.2.1)
		{
			head -n 2358 "${DISTDIR}/${P}-mozilla-kde.patch" || die "head failed"
			cat "${FILESDIR}/${P}-mozilla-kde_rebase.patch" || die "cat failed"
		} >"${T}/${P}-mozilla-kde.patch"
		PATCHES+=(
			"${T}/${P}-mozilla-kde.patch"
...
src_install() {
	...
	rm "${ED%/}${MOZILLA_FIVE_HOME}/thunderbird-bin" || die "rm failed"
